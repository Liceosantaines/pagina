...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('boletines-data.json')
      ...
      Object.entries(periodos).forEach(([pid, pName]) => {
        ...
        Object.entries(niveles).forEach(([slug, label]) => {
          ...
          if (Array.isArray(listaNivel) && listaNivel.length > 0) {
            ...
            li.innerHTML = `<a href="viewer.html?periodo=${pid}&nivel=${encodeURIComponent(slug)}"
    ...
    .catch(err => {
      ...
      grid.innerHTML = '<p class="error">No se pudieron cargar los boletines.</p>';
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('boletines-data.json')
    ...
    .then(data => {
      ...
      if (!lista.length) {
        grid.innerHTML = '<p class="error">No hay archivos para este nivel.</p>';
        return;
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Eventos básicos
  document.querySelector('.prev').addEventListener('click', () => moveToCard(currentIndex - 1));
  document.querySelector('.next').addEventListener('click', () => moveToCard(currentIndex + 1));
